...
**Разрешение имени идёт внешними программами** (`nslookup`, `getent`), а чужому процессу
`SO_MARK` не поставить. Значит DNS-запросы подборщика идут немаркированными — тем же путём,
что трафик человека. Если ваш продукт заворачивает `udp dport 53` в свою очередь, подбор
окажется внутри мира, который меряет. Обход на вашей стороне — правило по UID процесса.
