...
Замер на живой линии показал: одна маленькая проба, снятая один раз, — это шум. На
`/robots.txt` (1836 байт) проходило 21 стратегия, на реальной странице в 96 КБ — 10; два
одинаковых прогона одной и той же пробы дали 19 и 19 «рабочих», из которых общих — 11.
Объём завышает результат вдвое, а разброс без повтора неотличим от сигнала. `check`
поэтому ставит **две ПРОБЫ**, а не один запрос:
...
уже полного. Порядок задаёт `rank::fate_order`: частота полной доставки ↓ → медиана доли
↓ → ступень круга (`Good` → `Grinding` → не сузили → `Mirage` → `Trap` → `Dead`) →
простота тай-брейкером. Стратегии с полным кругом (`observed: "Unobserved"`) не
ранжируются вовсе и в список не попадают: это не последнее место, а отсутствие места.
`--take N` останавливает ПОИСК после N ПРОШЕДШИХ стратегий на протокол — выдачу он не
урезает, и если не прошёл никто, список всё равно выдаётся ранжированным, а не пустым.
