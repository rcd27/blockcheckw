...
**В отчёт идёт всякая НАБЛЮДЁННАЯ стратегия**, а не только приведшая цель к `Good`: та,
чей круг судеб уже полного. Порядок — по судьбе (`Good` → `Grinding` → не сузили →
`Mirage` → `Trap` → `Dead`), а не по длине строки аргументов. Стратегии с полным кругом
(`observed: "Unobserved"`) не ранжируются вовсе и в список не попадают: это не последнее
место, а отсутствие места. `--take N` останавливает ПОИСК после N стратегий с `Good` на
протокол — выдачу он не урезает, и если `Good` не нашлось ни у кого, список всё равно
выдаётся ранжированным, а не пустым.
...
поле `working` отчёта — счётчик строк с `Good`, а не длина списка. `median_speed_kbps` —
