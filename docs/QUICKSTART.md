...
`check` не отвечает «работает / не работает» — он сужает круг судеб цели: `Good` /
...
- Каждый прогон делает пробу без десинка (контроль). Если она сама доводит цель до
  `Good`, в отчёте стоит `inconclusive: true` — домен на этой линии не режется, и о
  стратегиях прогон не говорит ничего
