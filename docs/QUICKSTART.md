...
`scan` находит кандидатов, `check` проверяет их настоящей загрузкой и в конце печатает блок
`Рабочие стратегии` — готовые строки `nfqws2 …` по протоколам. Прокси и `jq` не нужны.
Подробности — [README](../README.md#tldr).
...
`check` ставит **две пробы** и не сливает их в один ответ (одна маленькая проба один раз
— шум: на живой линии два одинаковых прогона разошлись 16 стратегий из 27):

- **байтовая (главная)** — `--probe-path` (умолчание `/`), повторяется `M` раз
  (`--passes`, умолчание 3). Даёт долю `вытянуто/эталон` (`median_share`) и частоту
  полной доставки (`passes_ok`/`passes_total`) — сколько раз из `M` проба уложилась в
  собственный разброс эталона объёма;
- **подлинности (побочная)** — `--identity-path` (умолчание `/robots.txt`), один точный
  замер. Это круг судеб `admits`: `Good` / `Grinding` / `Mirage` / `Trap` / `Dead`
  (подробности — [README](../README.md#судьба-цели-observed-и-admits)). Только она может
  дать `Mirage`. Порядок выдачи: частота полной доставки → медиана доли → ступень круга
  → простота тай-брейкером.

**`working` — рабочая стратегия:**
...
не доставил страницу целиком: с `--reference-via` — не уложился в эталон объёма, без него
— сервер не закончил тело (потолок DPI, обрыв). Эталон для `working` не обязателен: без
него круг судеб честно остаётся широким, но рабочая стратегия находится.
...
круг из трёх судеб — но на `working` это не влияет.
...
- Каждый прогон делает обе пробы без десинка (контроль) и судит их ТОЙ ЖЕ мерой, что и
...
- Байтовая ось идёт не в статичный файл, а по `--probe-path` (умолчание `/`) — главный
  вердикт от пути не зависит (DPI решает по SNI в `ClientHello`, до HTTP), а объёму нужен
  настоящий ресурс. Ось подлинности идёт по `--identity-path` (умолчание `/robots.txt`)
  — детерминированный путь: он не редиректит и не гуляет в объёме между узлами CDN, и
  сверка с эталоном точна. Нет `robots.txt` — сервер отдаст `404`, и это полноценная
  проба: эталон получит тот же `404` и сойдётся
...
| `--reference-via <ENDPOINT>`  | Чистый egress для ОБОИХ эталонов. Без него `Good` не объявляется            |
| `--probe-path <PATH>`         | Путь байтовой оси (по умолчанию `/`), повторяется `--passes` раз            |
| `--identity-path <PATH>`      | Путь оси подлинности (по умолчанию `/robots.txt`), один точный замер        |
| `--passes <N>`                | `M` — сколько раз мерить байтовую ось (по умолчанию 3, `0` = не наблюдали)  |
