...
pub struct VerifiedStrategy {
    ...
    /// и то же, и `working: false` в обоих случаях означает разное.
    pub admits: Vec<String>,
    /// ГЛАВНАЯ ось замера, и она НЕ проекция круга судеб (спека §6-бис):
    /// `working = байты потекли И разговор не прервали И (эталона нет ИЛИ содержимое
    /// сошлось)`. `false` — только при положительном свидетельстве ПРОТИВ; неустановленная
    /// подлинность (широкий `admits` без эталона) вердикта о канале не отменяет.
    /// Считается `fate::passed`. По этому полю работает пайп `universal → check`.
    pub working: bool,
}
