...
///
/// # Почему её нельзя просто поднять
///
/// Мысль, приходящая здесь первой, — «пусть сосед сдвинет приборы с 13-го бита на 16-й, и
/// наш потолок вырастет до 65 535». Она не проходит, и разбор её стоит записать, иначе он
/// будет проделан заново каждым следующим читателем (сверено с владельцем невода 20.09.2026):
///
/// 1. Приборам нужно **15 бит одним куском** — `Layout::new` отвергает узкую и прерывистую
///    маску (тег 4 бита, фаза, оттиск). Подними их на 16 — получится 16..30, а бит 30 занят
///    маркой впрыска `reflex` (30..31).
/// 2. Цепочка не сходится **ни в одну сторону**: ужать впрыск до одного бита — внутрь полосы
///    попадают наши 28..29; сдвинуть наши 28..29 вниз — там наш же профиль; отдать приборам
///    0..14 — там профиль и слоты невода. Итого нужно 16 + 15 + 8 + 2 + 2 = 43 бита при 32
///    имеющихся.
/// 3. Помещается всё это только потому, что **слова РАЗНЫЕ**: часть величин живёт в марке
///    пакета, часть — в `ct mark` соединения. И наше пересечение с приборами по битам 13..15
///    реально ровно оттого, что индекс профиля и приборная памятка живут в ОДНОМ слове — в
///    марке пакета.
///
/// Третий пункт здесь главный. Без него первые два выглядят упрямством соседа, а с ним видно,
/// что потолок профиля — не уступка чужому удобству, а следствие разрядности.
