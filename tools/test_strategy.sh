...
NFQWS2="${NFQWS2:-/opt/zapret2/binaries/linux-x86_64/nfqws2}"
TABLE="${TABLE:-blockcheckw}"
QNUM=200
MARK=0x10000000
IP="104.21.32.39"
...
nft add chain inet $TABLE postnat '{ type filter hook postrouting priority 102; }' 2>/dev/null
nft add chain inet $TABLE predefrag '{ type filter hook output priority -402; }' 2>/dev/null
nft add rule inet $TABLE predefrag meta nfproto ipv4 mark and $MARK != 0 notrack 2>/dev/null
nft add chain inet $TABLE prenat '{ type filter hook prerouting priority -102; }' 2>/dev/null
...
OUT_HANDLE=$(nft --echo --handle add rule inet $TABLE postnat \
...
IN_HANDLE=$(nft --echo --handle add rule inet $TABLE prenat \
...
# 5. Cleanup
nft delete rule inet $TABLE postnat handle $OUT_HANDLE 2>/dev/null
nft delete rule inet $TABLE prenat handle $IN_HANDLE 2>/dev/null
